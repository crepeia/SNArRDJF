---
title: "SNA Edge Betwenness 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12)."
...
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
#Edge Betweenness
E(var12)$edge.betweenness_n<-edge.betweenness(var12, weights =E(var12)$equalone)
var12_edge.betweenness_n <-edge.betweenness(var12, weights =E(var12)$equalone)
```
#Edge Betweenness Descriptive
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
summary(var12_edge.betweenness_n)
sd(var12_edge.betweenness_n)
```
...
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
#Edge Betweenness
E(var12)$edge.betweenness<-edge.betweenness(var12, weights =E(var12)$var12)
...
var12_df_betweenness <- data.frame(var12_edge.betweenness_n ,
var12_edge.betweenness) %>% round(1)

labels<-attr(E(var12), "vnames")
#edgebt<-cbind(round(var12_edge.betweenness,2),labels)

#Adding type
var12_df_betweenness <-cbind(var12_df_betweenness, labels)

#Adding names
names(var12_df_betweenness) <- c("Edge Betweenness", "Edge Betweenness Weighted", "Type")

#Ordering Variables
var12_df_betweenness<-var12_df_betweenness[c("Type","Edge Betweenness", "Edge Betweenness Weighted")]
```
...
minC[1] <- maxC[1] <- 0
co <- layout_with_fr(var12, niter=10000, minx=minC, maxx=maxC,miny=minC, maxy=maxC, weights =E(var12)$edge.betweenness)

min(E(var12)$edge.betweenness)
#PLotting
plot(var12, 
     layout=co,
     edge.color=V(var12)$color[edge.start],
     edge.arrow.size=E(var12)$edge.betweenness/10000*mean(E(var12)$edge.betweenness+1),
     edge.width=E(var12)$var12/10*mean((E(var12)$var12+1)),
     edge.curved = TRUE,
...
#Adding Title
  title("Network Edge Betweenness Sized - 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).", sub = "Source: from authors ", cex = .5)
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Mean Betweenness : %.2f\n SD Betweenness : %.2f",
     mean(E(var12)$edge.betweenness), 
     sd(E(var12)$edge.betweenness)
   ))
```
...
save.image("~/SNArRDJF/Robject/11_ebetweenness_var12.RData") 
```
