---
title: "SNA Edge Betwenness 1.1 - REDE COMPLETA (full_no_zero)"
...
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
#Edge Betweenness
E(full_no_zero)$edge.betweenness_n<-edge.betweenness(full_no_zero, weights =E(full_no_zero)$equalone)
full_no_zero_edge.betweenness_n <-edge.betweenness(full_no_zero, weights =E(full_no_zero)$equalone)
```
#Edge Betweenness Descriptive
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
summary(full_no_zero_edge.betweenness_n)
sd(full_no_zero_edge.betweenness_n)
```
...
```{r, echo=TRUE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
#Edge Betweenness
E(full_no_zero)$edge.betweenness<-edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)
...
full_no_zero_df_betweenness <- data.frame(full_no_zero_edge.betweenness_n ,
full_no_zero_edge.betweenness) %>% round(1)

labels<-attr(E(full_no_zero), "vnames")
#edgebt<-cbind(round(full_no_zero_edge.betweenness,2),labels)

#Adding type
full_no_zero_df_betweenness <-cbind(full_no_zero_df_betweenness, labels)

#Adding names
names(full_no_zero_df_betweenness) <- c("Edge Betweenness", "Edge Betweenness Weighted", "Type")

#Ordering Variables
full_no_zero_df_betweenness<-full_no_zero_df_betweenness[c("Type","Edge Betweenness", "Edge Betweenness Weighted")]
```
...
minC[1] <- maxC[1] <- 0
co <- layout_with_fr(full_no_zero, niter=10000, minx=minC, maxx=maxC,miny=minC, maxy=maxC, weights =E(full_no_zero)$edge.betweenness)

min(E(full_no_zero)$edge.betweenness)
#PLotting
plot(full_no_zero, 
     layout=co,
     edge.color=V(full_no_zero)$color[edge.start],
     edge.arrow.size=E(full_no_zero)$edge.betweenness/10000*mean(E(full_no_zero)$edge.betweenness+1),
     edge.width=E(full_no_zero)$full_no_zero/10*mean((E(full_no_zero)$full_no_zero+1)),
     edge.curved = TRUE,
...
#Adding Title
  title("Network Edge Betweenness Sized - 1.1 - REDE COMPLETA (full_no_zero)", sub = "Source: from authors ", cex = .5)
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Mean Betweenness : %.2f\n SD Betweenness : %.2f",
     mean(E(full_no_zero)$edge.betweenness), 
     sd(E(full_no_zero)$edge.betweenness)
   ))
```
