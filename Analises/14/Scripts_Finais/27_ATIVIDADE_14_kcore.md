...
V(atividade)$coreness<-graph.coreness(atividade)
...
V(atividade)$color_bar<-color_bar[coreness]
```
...
plot(atividade, 
     layout=co,
     edge.color=V(atividade)$color_bar[edge.start],
     ...
     vertex.size=(degree(atividade)+10)/5,
     vertex.frame.color="black",
     vertex.label.color="black",
     vertex.label=get.vertex.attribute(atividade,"LABEL_COR"),
     vertex.label.cex=(degree(atividade)+0.01)/400,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2])
...
a<-V(atividade)$coreness
b<-V(atividade)$color_bar
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
...
#Adding Title
  title("Coreness Colored Graph - 27_ATIVIDADE", sub = "Source: from authors ", cex = .5)
  text( 
    x=range(co[,1])[1],
    y=range(co[,2])[1], 
...
![](27_ATIVIDADE_14_kcore_files/figure-html/unnamed-chunk-8-1.png)<!-- -->

#Saving objects with new variables and changes

```r
save.image("~/SNArRDJF/Robject/14_kcore_atividade.RData") 
```
