---
title: "SNA Interactive Plotting 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)"
...
load("~/SNArRDJF/Robject/var11.RData")
```
##Reload packages
...
#Removing CAPS AD
```{r}
#make_ego_graph_all_var11_without_CAPSAD<-delete_vertices(var11,V(var11)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"])
#var11<-make_ego_graph_all_var11_without_CAPSAD
```
...
```
#Interactive Plotting - 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
