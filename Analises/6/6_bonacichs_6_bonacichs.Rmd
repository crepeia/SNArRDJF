...
##Power (beta) Centrality Stats - (positive attenuation factor). 
```{r, results="asis"}
d=as.data.frame(cbind(V(full_no_zero)$TIPO1,round(full_no_zero_power_centrality_stats,2)))
names(d)<-c("Type", "Power(beta)")
datatable(d, filter ="top")
```
...
##Power (beta) Centrality Stats - (positive attenuation factor). 
```{r, results="asis"}
d=as.data.frame(cbind(V(full_no_zero)$TIPO2,round(full_no_zero_power_centrality_stats,2)))
names(d)<-c("Type", "Power(beta)")
datatable(d, filter ="top")
```
...
V(full_no_zero)$full_no_zero_power_centrality_stats <-try(power_centrality(full_no_zero, nodes = V(full_no_zero), loops = FALSE, exponent = 0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))
...
plot(full_no_zero, 
     layout=co,
     edge.color=V(full_no_zero)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(full_no_zero)$full_no_zero_power_centrality_stats_r+0.001)^2)/10,
     edge.width=E(full_no_zero)$weight/(10*mean(E(full_no_zero)$weight)),
     edge.curved = TRUE,
     vertex.color=V(full_no_zero)$color_spower, 
     vertex.size=sqrt(((V(full_no_zero)$full_no_zero_power_centrality_stats_r+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(full_no_zero)$full_no_zero_power_centrality_stats_r+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Positive Attenuation - fuull_no_zero_fancy", sub = "Source: from authors ")
...
##Power (beta) Centrality Stats - (negative attenuation factor).
```{r, results="asis"}
d=as.data.frame(cbind(V(full_no_zero)$TIPO1,round(full_no_zero_power_centrality_stats_n,2)))
names(d)<-c("Type", "Power(beta)")
datatable(d, filter ="top")
```
...
##Power (beta) Centrality Stats - (negative attenuation factor).
```{r, results="asis"}
d=as.data.frame(cbind(V(full_no_zero)$TIPO2,round(full_no_zero_power_centrality_stats_n,2)))
names(d)<-c("Type", "Power(beta)")
datatable(d, filter ="top")
```
...
V(full_no_zero)$full_no_zero_power_centrality_stats_n <-try(power_centrality(full_no_zero, nodes = V(full_no_zero), loops = FALSE, exponent = -0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))

#Setting color for each vertex based on Power
V(full_no_zero)$full_no_zero_power_centrality_stats_r_n<-round(V(full_no_zero)$full_no_zero_power_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(full_no_zero)$full_no_zero_power_centrality_stats_r_n)), "YlOrRd"))(length(unique(V(full_no_zero)$full_no_zero_power_centrality_stats_r_n))) 

#Saving as Vertex properties                             
V(full_no_zero)$color_spower <- vertex_col_spower[factor(V(full_no_zero)$full_no_zero_power_centrality_stats_r_n)]  
...
plot(full_no_zero, 
     layout=co,
     edge.color=V(full_no_zero)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(full_no_zero)$full_no_zero_power_centrality_stats_r_n+0.001)^2)/10,
     edge.width=E(full_no_zero)$weight/(10*mean(E(full_no_zero)$weight)),
     edge.curved = TRUE,
     vertex.color=V(full_no_zero)$color_spower, 
     vertex.size=sqrt(((V(full_no_zero)$full_no_zero_power_centrality_stats_r_n+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(full_no_zero)$full_no_zero_power_centrality_stats_r_n+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_power_centrality_stats_r_n
b<-V(full_no_zero)$color_spower
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Negative Attenuation - fuull_no_zero_fancy", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Power Degree:%.2f\nSD Power Degree:%.2f",
     median(full_no_zero_power_centrality_stats_n),sd(full_no_zero_power_centrality_stats_n)
   ))
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$LABEL_COR, round(as.numeric(full_no_zero_alpha_centrality_stats),2)))
...
##Alpha Bonacich's Table - positive attenuation factor
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$TIPO1, round(as.numeric(full_no_zero_alpha_centrality_stats),2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
##Alpha Bonacich's Table - positive attenuation factor
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$TIPO2, round(as.numeric(full_no_zero_alpha_centrality_stats),2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
V(full_no_zero)$full_no_zero_alpha_centrality_stats_r<-round(as.numeric(V(full_no_zero)$full_no_zero_alpha_centrality_stats),0)
...
plot(full_no_zero, 
     layout=co,
     edge.color=V(full_no_zero)$color_s[edge.start],
     edge.arrow.size=sqrt((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r+0.0001)^2)/50,
     edge.width=E(full_no_zero)$weight/(10*mean(E(full_no_zero)$weight)),
     edge.curved = TRUE,
     vertex.color=V(full_no_zero)$color_s, 
     vertex.size=sqrt(((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r+0.0001)^2))*2,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r+0.0001)^2)/50,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Positive Attenuation - fuull_no_zero_fancy", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$LABEL_COR, round(full_no_zero_alpha_centrality_stats_n,2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
##Alpha Bonacich's Table - negative attenuation factor
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$TIPO1, round(full_no_zero_alpha_centrality_stats_n,2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
##Alpha Bonacich's Table - negative attenuation factor
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(full_no_zero)$TIPO2, round(full_no_zero_alpha_centrality_stats_n,2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
# Set colors to plot distances:
V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n<-round(V(full_no_zero)$full_no_zero_alpha_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n)), "Blues"))(length(unique(V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n))) 

#Saving as Vertex properties                                                                  
V(full_no_zero)$color_s <- vertex_col_s[factor(V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n)]  
...
plot(full_no_zero, 
     layout=co,
     edge.color=V(full_no_zero)$color_s[edge.start],
     edge.arrow.size=sqrt((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n+0.0001)^2)/10,
     edge.width=E(full_no_zero)$weight/(10*mean(E(full_no_zero)$weight)),
     edge.curved = TRUE,
     vertex.color=V(full_no_zero)$color_s, 
     vertex.size=sqrt(((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n+0.0001)^2))*5,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n+0.0001)^2)/25,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_alpha_centrality_stats_r_n
b<-V(full_no_zero)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Negative Attenuation - fuull_no_zero_fancy", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(full_no_zero_alpha_centrality_stats_n),sd(full_no_zero_alpha_centrality_stats_n)
   ))
```
