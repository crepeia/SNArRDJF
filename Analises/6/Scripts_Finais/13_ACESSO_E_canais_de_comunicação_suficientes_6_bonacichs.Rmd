---
title: "SNA Bonacich's 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)"
...
V(var11)$var11_power_centrality_stats <-try(power_centrality(var11, nodes = V(var11), loops = FALSE, exponent = 0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))
...
plot(var11, 
     layout=co,
     edge.color=V(var11)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(var11)$var11_power_centrality_stats_r+0.001)^2)/10,
     edge.width=E(var11)$weight/(10*mean(E(var11)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var11)$color_spower, 
     vertex.size=sqrt(((V(var11)$var11_power_centrality_stats_r+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var11)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var11)$var11_power_centrality_stats+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Positive Attenuation - 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)", sub = "Source: from authors ")
...
V(var11)$var11_power_centrality_stats_n <-try(power_centrality(var11, nodes = V(var11), loops = FALSE, exponent = -0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))

#Setting color for each vertex based on Power
V(var11)$var11_power_centrality_stats_r_n<-round(V(var11)$var11_power_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(var11)$var11_power_centrality_stats_r_n)), "YlOrRd"))(length(unique(V(var11)$var11_power_centrality_stats_r_n))) 

#Saving as Vertex properties                             
V(var11)$color_spower <- vertex_col_spower[factor(V(var11)$var11_power_centrality_stats_r_n)]  
...
plot(var11, 
     layout=co,
     edge.color=V(var11)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(var11)$var11_power_centrality_stats_r_n+0.001)^2)/10,
     edge.width=E(var11)$weight/(10*mean(E(var11)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var11)$color_spower, 
     vertex.size=sqrt(((V(var11)$var11_power_centrality_stats_r_n+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var11)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var11)$var11_power_centrality_stats_r_n+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(var11)$var11_power_centrality_stats_r_n
b<-V(var11)$color_spower
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Negative Attenuation - 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Power Degree:%.2f\nSD Power Degree:%.2f",
     median(var11_power_centrality_stats_n),sd(var11_power_centrality_stats_n)
   ))
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(var11)$LABEL_COR, round(var11_alpha_centrality_stats,2)))
...
plot(var11, 
     layout=co,
     edge.color=V(var11)$color_s[edge.start],
     edge.arrow.size=sqrt((V(var11)$var11_alpha_centrality_stats_r+0.0001)^2)/50,
     edge.width=E(var11)$weight/(10*mean(E(var11)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var11)$color_s, 
     vertex.size=sqrt(((V(var11)$var11_alpha_centrality_stats_r+0.0001)^2))*2,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var11)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var11)$var11_alpha_centrality_stats_r+0.0001)^2)/50,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Positive Attenuation - 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(var11)$LABEL_COR, round(var11_alpha_centrality_stats_n,2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
# Set colors to plot distances:
V(var11)$var11_alpha_centrality_stats_r_n<-round(V(var11)$var11_alpha_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(var11)$var11_alpha_centrality_stats_r_n)), "Blues"))(length(unique(V(var11)$var11_alpha_centrality_stats_r_n))) 

#Saving as Vertex properties                                                                  
V(var11)$color_s <- vertex_col_s[factor(V(var11)$var11_alpha_centrality_stats_r_n)]  
...
plot(var11, 
     layout=co,
     edge.color=V(var11)$color_s[edge.start],
     edge.arrow.size=sqrt((V(var11)$var11_alpha_centrality_stats_r_n+0.0001)^2)/10,
     edge.width=E(var11)$weight/(10*mean(E(var11)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var11)$color_s, 
     vertex.size=sqrt(((V(var11)$var11_alpha_centrality_stats_r_n+0.0001)^2))*5,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var11)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var11)$var11_alpha_centrality_stats_r_n+0.0001)^2)/25,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(var11)$var11_alpha_centrality_stats_r_n
b<-V(var11)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Negative Attenuation - 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(var11_alpha_centrality_stats_n),sd(var11_alpha_centrality_stats_n)
   ))
```
