---
title: "SNA Bonacich's 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12)."
...
V(var12)$var12_power_centrality_stats <-try(power_centrality(var12, nodes = V(var12), loops = FALSE, exponent = 0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))
...
plot(var12, 
     layout=co,
     edge.color=V(var12)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(var12)$var12_power_centrality_stats_r+0.001)^2)/10,
     edge.width=E(var12)$weight/(10*mean(E(var12)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var12)$color_spower, 
     vertex.size=sqrt(((V(var12)$var12_power_centrality_stats_r+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var12)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var12)$var12_power_centrality_stats+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Positive Attenuation - 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).", sub = "Source: from authors ")
...
V(var12)$var12_power_centrality_stats_n <-try(power_centrality(var12, nodes = V(var12), loops = FALSE, exponent = -0.5, rescale = FALSE, tol = 1e-07, sparse = TRUE))

#Setting color for each vertex based on Power
V(var12)$var12_power_centrality_stats_r_n<-round(V(var12)$var12_power_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(var12)$var12_power_centrality_stats_r_n)), "YlOrRd"))(length(unique(V(var12)$var12_power_centrality_stats_r_n))) 

#Saving as Vertex properties                             
V(var12)$color_spower <- vertex_col_spower[factor(V(var12)$var12_power_centrality_stats_r_n)]  
...
plot(var12, 
     layout=co,
     edge.color=V(var12)$color_spower[edge.start],
     edge.arrow.size=sqrt((V(var12)$var12_power_centrality_stats_r_n+0.001)^2)/10,
     edge.width=E(var12)$weight/(10*mean(E(var12)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var12)$color_spower, 
     vertex.size=sqrt(((V(var12)$var12_power_centrality_stats_r_n+0.001)^2))*15,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var12)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var12)$var12_power_centrality_stats_r_n+0.001)^2)/5,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(var12)$var12_power_centrality_stats_r_n
b<-V(var12)$color_spower
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - Negative Attenuation - 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Power Degree:%.2f\nSD Power Degree:%.2f",
     median(var12_power_centrality_stats_n),sd(var12_power_centrality_stats_n)
   ))
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(var12)$LABEL_COR, round(var12_alpha_centrality_stats,2)))
...
plot(var12, 
     layout=co,
     edge.color=V(var12)$color_s[edge.start],
     edge.arrow.size=sqrt((V(var12)$var12_alpha_centrality_stats_r+0.0001)^2)/50,
     edge.width=E(var12)$weight/(10*mean(E(var12)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var12)$color_s, 
     vertex.size=sqrt(((V(var12)$var12_alpha_centrality_stats_r+0.0001)^2))*2,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var12)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var12)$var12_alpha_centrality_stats_r+0.0001)^2)/50,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Positive Attenuation - 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
d=as.data.frame(cbind(V(var12)$LABEL_COR, round(var12_alpha_centrality_stats_n,2)))
names(d)<-c("Type", "Alpha Bonacich")
datatable(d, filter ="top")
```
...
```{r, echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
# Set colors to plot distances:
V(var12)$var12_alpha_centrality_stats_r_n<-round(V(var12)$var12_alpha_centrality_stats_n,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(var12)$var12_alpha_centrality_stats_r_n)), "Blues"))(length(unique(V(var12)$var12_alpha_centrality_stats_r_n))) 

#Saving as Vertex properties                                                                  
V(var12)$color_s <- vertex_col_s[factor(V(var12)$var12_alpha_centrality_stats_r_n)]  
...
plot(var12, 
     layout=co,
     edge.color=V(var12)$color_s[edge.start],
     edge.arrow.size=sqrt((V(var12)$var12_alpha_centrality_stats_r_n+0.0001)^2)/10,
     edge.width=E(var12)$weight/(10*mean(E(var12)$weight)),
     edge.curved = TRUE,
     vertex.color=V(var12)$color_s, 
     vertex.size=sqrt(((V(var12)$var12_alpha_centrality_stats_r_n+0.0001)^2))*5,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label=V(var12)$V5_LABEL_LABEL,
     vertex.label.cex=sqrt((V(var12)$var12_alpha_centrality_stats_r_n+0.0001)^2)/25,
     vertex.label.dist=0,
     rescale=F,
     xlim=range(co[,1]), 
     ylim=range(co[,2]))
...
#Solving Problems with legend rendering 
a<-V(var12)$var12_alpha_centrality_stats_r_n
b<-V(var12)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - Negative Attenuation - 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).", sub = "Source: from authors ")
  text(x=range(co[,1])[1], y=range(co[,2])[1], labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(var12_alpha_centrality_stats_n),sd(var12_alpha_centrality_stats_n)
   ))
```
