# SNA Reachiability 26_CONFIANÇA I) Espera que o relacionamento com este serviço se fortaleça nos próximos anos. (var24)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var24.RData")
```
##Reload packages
...
V(var24)$reach_in<-var24_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var24)$reach_in <-mean(var24_reach_in[which(var24_reach_in != Inf)])
...
#Adding to igraph object
V(var24)$reach_out<-var24_reach_out_vec
```
...
var24_reachability_df <- data.frame(var24_reach_in_vec,var24_reach_out_vec) %>% round(3)

#Adding type
var24_reachability_df <-cbind(var24_reachability_df, V(var24)$LABEL_COR)

#Adding names
names(var24_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var24_reachability_df<-var24_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var24_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var24_reachability_df, by=list(var24_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var24_reachability_df, by=list(var24_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var24.RData") 
```
