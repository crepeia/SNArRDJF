# SNA Reachiability 23_CONFIANÇA F) Confia em realizar ações conjuntas com este serviço (var21).
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var21.RData")
```
##Reload packages
...
V(var21)$reach_in<-var21_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var21)$reach_in <-mean(var21_reach_in[which(var21_reach_in != Inf)])
...
#Adding to igraph object
V(var21)$reach_out<-var21_reach_out_vec
```
...
var21_reachability_df <- data.frame(var21_reach_in_vec,var21_reach_out_vec) %>% round(3)

#Adding type
var21_reachability_df <-cbind(var21_reachability_df, V(var21)$LABEL_COR)

#Adding names
names(var21_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var21_reachability_df<-var21_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var21_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var21_reachability_df, by=list(var21_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var21_reachability_df, by=list(var21_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var21.RData") 
```
