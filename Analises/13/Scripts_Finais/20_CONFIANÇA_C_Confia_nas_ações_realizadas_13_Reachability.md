# SNA Reachiability 20_CONFIANÇA C) Confia nas ações realizadas por este serviço. (var18)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var18.RData")
```
##Reload packages
...
V(var18)$reach_in<-var18_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var18)$reach_in <-mean(var18_reach_in[which(var18_reach_in != Inf)])
...
#Adding to igraph object
V(var18)$reach_out<-var18_reach_out_vec
```
...
var18_reachability_df <- data.frame(var18_reach_in_vec,var18_reach_out_vec) %>% round(3)

#Adding type
var18_reachability_df <-cbind(var18_reachability_df, V(var18)$LABEL_COR)

#Adding names
names(var18_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var18_reachability_df<-var18_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var18_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var18_reachability_df, by=list(var18_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var18_reachability_df, by=list(var18_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var18.RData") 
```
