# SNA Reachiability 22_CONFIANÇA E) Confia nas informações recebas deste serviço (var20).
Leonardo Martins  
17 de julho de 2016  
...
V(var20)$reach_in<-var20_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var20)$reach_in <-mean(var20_reach_in[which(var20_reach_in != Inf)])
...
#Adding to igraph object
V(var20)$reach_out<-var20_reach_out_vec
```
...
var20_reachability_df <- data.frame(var20_reach_in_vec,var20_reach_out_vec) %>% round(3)

#Adding type
var20_reachability_df <-cbind(var20_reachability_df, V(var20)$LABEL_COR)

#Adding names
names(var20_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var20_reachability_df<-var20_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var20_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var20_reachability_df, by=list(var20_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var20_reachability_df, by=list(var20_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var20.RData") 
```
