# SNA Reachiability 19_CONFIANÇA B) Confia na capacade técnica da equipe deste serviço. (var17)
Leonardo Martins  
17 de julho de 2016  
...
V(var17)$reach_in<-var17_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var17)$reach_in <-mean(var17_reach_in[which(var17_reach_in != Inf)])
...
#Adding to igraph object
V(var17)$reach_out<-var17_reach_out_vec
```
...
var17_reachability_df <- data.frame(var17_reach_in_vec,var17_reach_out_vec) %>% round(3)

#Adding type
var17_reachability_df <-cbind(var17_reachability_df, V(var17)$LABEL_COR)

#Adding names
names(var17_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var17_reachability_df<-var17_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var17_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var17_reachability_df, by=list(var17_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var17_reachability_df, by=list(var17_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var17.RData") 
```
