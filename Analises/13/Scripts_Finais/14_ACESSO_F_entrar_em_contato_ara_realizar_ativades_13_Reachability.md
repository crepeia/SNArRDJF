# SNA Reachiability 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var12.RData")
```
##Reload packages
...
V(var12)$reach_in<-var12_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var12)$reach_in <-mean(var12_reach_in[which(var12_reach_in != Inf)])
...
#Adding to igraph object
V(var12)$reach_out<-var12_reach_out_vec
```
...
var12_reachability_df <- data.frame(var12_reach_in_vec,var12_reach_out_vec) %>% round(3)

#Adding type
var12_reachability_df <-cbind(var12_reachability_df, V(var12)$LABEL_COR)

#Adding names
names(var12_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var12_reachability_df<-var12_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var12_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var12_reachability_df, by=list(var12_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var12_reachability_df, by=list(var12_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var12.RData") 
```
