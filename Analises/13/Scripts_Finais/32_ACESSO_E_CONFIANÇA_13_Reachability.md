# SNA Reachiability 32_ACESSO E CONFIANÇA
Leonardo Martins  
17 de julho de 2016  
...
acessoeconfianca_reach_in_vec <- vector()
for (i in 1:vcount(acessoeconfianca)) {
    acessoeconfianca_reach_in_vec[i] <- mean(acessoeconfianca_reach_in[i,])
}

#Adding to igraph object
V(acessoeconfianca)$reach_in<-acessoeconfianca_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(acessoeconfianca)$reach_in <-mean(acessoeconfianca_reach_in[which(acessoeconfianca_reach_in != Inf)])
...
#Mean Reachbility by Vertex
acessoeconfianca_reach_out_vec <- vector()
for (i in 1:vcount(acessoeconfianca)) {
    acessoeconfianca_reach_out_vec[i] <- mean(acessoeconfianca_reach_out[i,])
}

#Adding to igraph object
V(acessoeconfianca)$reach_out<-acessoeconfianca_reach_out_vec
```
...
acessoeconfianca_reachability_df <- data.frame(acessoeconfianca_reach_in_vec,acessoeconfianca_reach_out_vec) %>% round(3)

#Adding type
acessoeconfianca_reachability_df <-cbind(acessoeconfianca_reachability_df, V(acessoeconfianca)$LABEL_COR)

#Adding names
names(acessoeconfianca_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
acessoeconfianca_reachability_df<-acessoeconfianca_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(acessoeconfianca_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(acessoeconfianca_reachability_df, by=list(acessoeconfianca_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(acessoeconfianca_reachability_df, by=list(acessoeconfianca_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_acessoeconfianca.RData") 
```
