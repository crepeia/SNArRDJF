# SNA Reachiability 11_ACESSO C) Este serviço disponibiliza informações sobre suas ativades para o seu serviço. (var9)
Leonardo Martins  
17 de julho de 2016  
...
V(var9)$reach_in<-var9_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var9)$reach_in <-mean(var9_reach_in[which(var9_reach_in != Inf)])
...
#Adding to igraph object
V(var9)$reach_out<-var9_reach_out_vec
```
...
var9_reachability_df <- data.frame(var9_reach_in_vec,var9_reach_out_vec) %>% round(3)

#Adding type
var9_reachability_df <-cbind(var9_reachability_df, V(var9)$LABEL_COR)

#Adding names
names(var9_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var9_reachability_df<-var9_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var9_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var9_reachability_df, by=list(var9_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var9_reachability_df, by=list(var9_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var9.RData") 
```
