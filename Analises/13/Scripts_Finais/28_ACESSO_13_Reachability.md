...
load("~/SNArRDJF/Robject/12_cliques_acesso.RData")
```
##Reload packages
...
V(acesso)$reach_in<-acesso_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(acesso)$reach_in <-mean(acesso_reach_in[which(acesso_reach_in != Inf)])
...
#Adding to igraph object
V(acesso)$reach_out<-acesso_reach_out_vec
```
...
acesso_reachability_df <- data.frame(acesso_reach_in_vec,acesso_reach_out_vec) %>% round(3)

#Adding type
acesso_reachability_df <-cbind(acesso_reachability_df, V(acesso)$LABEL_COR)

#Adding names
names(acesso_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
acesso_reachability_df<-acesso_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(acesso_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(acesso_reachability_df, by=list(acesso_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(acesso_reachability_df, by=list(acesso_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_acesso.RData") 
```
