# SNA Reachiability 10_ACESSO B) O horário de funcionamento deste serviço possibilita a realização de ações em conjunto. (var8)
Leonardo Martins  
17 de julho de 2016  
...
V(var8)$reach_in<-var8_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var8)$reach_in <-mean(var8_reach_in[which(var8_reach_in != Inf)])
...
#Adding to igraph object
V(var8)$reach_out<-var8_reach_out_vec
```
...
```r
aggdata_sd <-aggregate(var8_reachability_df, by=list(var8_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var8.RData") 
```
