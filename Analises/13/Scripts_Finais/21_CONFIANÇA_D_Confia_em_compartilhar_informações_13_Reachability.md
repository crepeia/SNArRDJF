# SNA Reachiability 21_CONFIANÇA D) Confia em compartilhar informações internas com este serviço. (var19) 
Leonardo Martins  
17 de julho de 2016  
...
V(var19)$reach_in<-var19_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var19)$reach_in <-mean(var19_reach_in[which(var19_reach_in != Inf)])
...
#Adding to igraph object
V(var19)$reach_out<-var19_reach_out_vec
```
...
var19_reachability_df <- data.frame(var19_reach_in_vec,var19_reach_out_vec) %>% round(3)

#Adding type
var19_reachability_df <-cbind(var19_reachability_df, V(var19)$LABEL_COR)

#Adding names
names(var19_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var19_reachability_df<-var19_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var19_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var19_reachability_df, by=list(var19_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var19_reachability_df, by=list(var19_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var19.RData") 
```
