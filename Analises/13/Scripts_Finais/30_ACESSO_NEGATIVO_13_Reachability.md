# SNA Reachiability 30_ACESSO_NEGATIVO
Leonardo Martins  
17 de julho de 2016  
...
acesso_negativo_reach_in_vec <- vector()
for (i in 1:vcount(acesso_negativo)) {
    acesso_negativo_reach_in_vec[i] <- mean(acesso_negativo_reach_in[i,])
}

#Adding to igraph object
V(acesso_negativo)$reach_in<-acesso_negativo_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(acesso_negativo)$reach_in <-mean(acesso_negativo_reach_in[which(acesso_negativo_reach_in != Inf)])
...
#Mean Reachbility by Vertex
acesso_negativo_reach_out_vec <- vector()
for (i in 1:vcount(acesso_negativo)) {
    acesso_negativo_reach_out_vec[i] <- mean(acesso_negativo_reach_out[i,])
}

#Adding to igraph object
V(acesso_negativo)$reach_out<-acesso_negativo_reach_out_vec
```
...
acesso_negativo_reachability_df <- data.frame(acesso_negativo_reach_in_vec,acesso_negativo_reach_out_vec) %>% round(3)

#Adding type
acesso_negativo_reachability_df <-cbind(acesso_negativo_reachability_df, V(acesso_negativo)$LABEL_COR)

#Adding names
names(acesso_negativo_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
acesso_negativo_reachability_df<-acesso_negativo_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(acesso_negativo_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(acesso_negativo_reachability_df, by=list(acesso_negativo_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(acesso_negativo_reachability_df, by=list(acesso_negativo_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_acesso_negativo.RData") 
```
