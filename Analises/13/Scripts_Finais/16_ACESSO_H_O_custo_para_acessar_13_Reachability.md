# SNA Reachiability 16_ACESSO H) O custo para acessar este serviço impede a realização de ativades em conjunto. (var14)
Leonardo Martins  
17 de julho de 2016  
...
V(var14)$reach_in<-var14_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var14)$reach_in <-mean(var14_reach_in[which(var14_reach_in != Inf)])
...
#Adding to igraph object
V(var14)$reach_out<-var14_reach_out_vec
```
...
var14_reachability_df <- data.frame(var14_reach_in_vec,var14_reach_out_vec) %>% round(3)

#Adding type
var14_reachability_df <-cbind(var14_reachability_df, V(var14)$LABEL_COR)

#Adding names
names(var14_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var14_reachability_df<-var14_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var14_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var14_reachability_df, by=list(var14_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var14_reachability_df, by=list(var14_reachability_df$Type), FUN=sd, na.rm=TRUE) 
