# SNA Reachiability 9_ACESSO A) Este serviço mostra-se disponível para realização de ativades em conjunto. (var7)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var7.RData")
```
##Reload packages
...
V(var7)$reach_in<-var7_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var7)$reach_in <-mean(var7_reach_in[which(var7_reach_in != Inf)])
...
#Adding to igraph object
V(var7)$reach_out<-var7_reach_out_vec
```
...
var7_reachability_df <- data.frame(var7_reach_in_vec,var7_reach_out_vec) %>% round(3)

#Adding type
var7_reachability_df <-cbind(var7_reachability_df, V(var7)$LABEL_COR)

#Adding names
names(var7_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var7_reachability_df<-var7_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var7_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var7_reachability_df, by=list(var7_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var7_reachability_df, by=list(var7_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var7.RData") 
```
