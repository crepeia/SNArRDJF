# SNA Reachiability 12_ACESSO D) O seu serviço consegue acessar facilmente informações deste serviço. (var10)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var10.RData")
```
##Reload packages
...
V(var10)$reach_in<-var10_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var10)$reach_in <-mean(var10_reach_in[which(var10_reach_in != Inf)])
...
#Adding to igraph object
V(var10)$reach_out<-var10_reach_out_vec
```
...
var10_reachability_df <- data.frame(var10_reach_in_vec,var10_reach_out_vec) %>% round(3)

#Adding type
var10_reachability_df <-cbind(var10_reachability_df, V(var10)$LABEL_COR)

#Adding names
names(var10_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var10_reachability_df<-var10_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var10_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var10_reachability_df, by=list(var10_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var10_reachability_df, by=list(var10_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var10.RData") 
```
