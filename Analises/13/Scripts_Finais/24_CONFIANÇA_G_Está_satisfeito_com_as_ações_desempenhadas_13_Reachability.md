# SNA Reachiability 24_CONFIANÇA G) Está satisfeito com as ações desempenhadas por este serviço (var22).
Leonardo Martins  
17 de julho de 2016  
...
V(var22)$reach_in<-var22_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var22)$reach_in <-mean(var22_reach_in[which(var22_reach_in != Inf)])
...
#Adding to igraph object
V(var22)$reach_out<-var22_reach_out_vec
```
...
var22_reachability_df <- data.frame(var22_reach_in_vec,var22_reach_out_vec) %>% round(3)

#Adding type
var22_reachability_df <-cbind(var22_reachability_df, V(var22)$LABEL_COR)

#Adding names
names(var22_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var22_reachability_df<-var22_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var22_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var22_reachability_df, by=list(var22_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var22_reachability_df, by=list(var22_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var22.RData") 
```
