# SNA Reachiability 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/12_cliques_var11.RData")
```
##Reload packages
...
V(var11)$reach_in<-var11_reach_in_vec
```
#Descriptive - Reachability - IN

```r
V(var11)$reach_in <-mean(var11_reach_in[which(var11_reach_in != Inf)])
...
#Adding to igraph object
V(var11)$reach_out<-var11_reach_out_vec
```
...
var11_reachability_df <- data.frame(var11_reach_in_vec,var11_reach_out_vec) %>% round(3)

#Adding type
var11_reachability_df <-cbind(var11_reachability_df, V(var11)$LABEL_COR)

#Adding names
names(var11_reachability_df) <- c("Reach IN", "Reach OUT","Type")

#Ordering Variables
var11_reachability_df<-var11_reachability_df[c("Type", "Reach IN", "Reach OUT")]
```
## General tabel - DT 

```r
datatable(var11_reachability_df, filter = 'top')
```
...
```r
aggdata_mean <-aggregate(var11_reachability_df, by=list(var11_reachability_df$Type), FUN=mean, na.rm=TRUE)
...
```r
aggdata_sd <-aggregate(var11_reachability_df, by=list(var11_reachability_df$Type), FUN=sd, na.rm=TRUE) 
...
```r
save.image("~/SNArRDJF/Robject/13_reach_var11.RData") 
```
